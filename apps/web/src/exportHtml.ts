...
export function exportDiagramAsHtml(nodes: Node[], edges: Edge[], title: string) {
  // Build a minimal graph from current nodes/edges for the full exporter
...
<div id="app">
  <header class="hdr">
    ...
    <div class="hdr-info">
      <span id="stats"></span>
      <span class="sep">·</span>
      <span>Exportiert am ${esc(exportDate)}</span>
    </div>
  </header>
...
${RUNTIME_JS}
  ...
  a.href = url;
  a.download = `${projectName.replace(/[^a-zA-Z0-9_-]/g, "_")}_full.html`;
  a.click();
...
/* ═══════════════════════════════════════════════════
   CSS — all styles inlined
   ═══════════════════════════════════════════════════ */

const CSS_CONTENT = `
*{box-sizing:border-box;margin:0;padding:0}
:root{
  --bg:#0f1117;--bgp:#1a1d27;--bgc:#23263a;--bdr:#2d3148;
  --txt:#e2e4f0;--dim:#8b8fa7;--acc:#6c8cff;--acch:#8ba5ff;
  --yel:#ffd866;--grn:#80e0a0;--org:#ffab70;--red:#ff6b6b;
}
body{background:var(--bg);color:var(--txt);font-family:"Inter",system-ui,sans-serif;height:100vh;overflow:hidden}
#app{display:grid;grid-template-columns:240px 1fr 320px;grid-template-rows:44px 1fr;grid-template-areas:"hdr hdr hdr" "sb vp insp";height:100vh}
.hdr{grid-area:hdr;display:flex;align-items:center;gap:12px;padding:0 16px;background:var(--bgp);border-bottom:1px solid var(--bdr);z-index:20}
.hdr h1{font-size:15px;font-weight:600;color:var(--acc)}
.hdr-info{margin-left:auto;font-size:11px;color:var(--dim);display:flex;align-items:center;gap:6px}
.hdr-info .sep{color:var(--bdr)}

/* Breadcrumb */
.breadcrumb{display:flex;align-items:center;gap:4px;font-size:13px;color:var(--dim)}
.breadcrumb button{background:none;border:none;color:var(--acc);cursor:pointer;font-size:13px;padding:2px 4px;border-radius:4px}
.breadcrumb button:hover{background:rgba(108,140,255,.15)}
.breadcrumb .sep{color:var(--dim);margin:0 2px}

/* Sidebar */
.sb{grid-area:sb;background:var(--bgp);border-right:1px solid var(--bdr);overflow-y:auto;padding:12px}
.sb h2{font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);margin-bottom:8px}
.sb::-webkit-scrollbar{width:5px}
.sb::-webkit-scrollbar-thumb{background:var(--bdr);border-radius:4px}

.vt-item{display:flex;align-items:center;gap:4px;padding:4px 6px;border-radius:4px;cursor:pointer;font-size:12px;color:var(--txt);white-space:nowrap;overflow:hidden;transition:background .1s}
.vt-item:hover{background:rgba(108,140,255,.1)}
.vt-item.active{background:rgba(108,140,255,.18);color:var(--acc);font-weight:600}
.vt-chv{width:14px;font-size:10px;flex-shrink:0;text-align:center;color:var(--dim);cursor:pointer;transition:transform .15s;user-select:none}
...
.vt-children{margin-left:12px;border-left:1px solid var(--bdr);padding-left:4px}
...
.zoom-controls button{width:32px;height:32px;border:1px solid var(--bdr);background:var(--bgp);color:var(--txt);font-size:16px;cursor:pointer;border-radius:4px;display:flex;align-items:center;justify-content:center}
.zoom-controls button:hover{background:var(--bgc);color:var(--acc)}

/* Minimap */
.minimap{position:absolute;bottom:16px;left:16px;width:160px;height:100px;background:var(--bgp);border:1px solid var(--bdr);border-radius:6px;overflow:hidden;z-index:10;opacity:.7;pointer-events:none}
.minimap canvas{width:100%;height:100%}

/* ── Nodes ── */
.node{position:absolute;background:var(--bgc);border:2px solid var(--bdr);border-radius:8px;min-width:160px;font-size:13px;box-shadow:0 2px 8px rgba(0,0,0,.3);cursor:pointer;transition:border-color .15s,box-shadow .15s;user-select:none}
.node:hover,.node.selected{border-color:var(--acc);box-shadow:0 0 0 2px rgba(108,140,255,.25),0 4px 16px rgba(0,0,0,.3)}
.node-hdr{padding:8px 12px;border-bottom:1px solid var(--bdr);display:flex;align-items:center;gap:6px}
.kbadge{font-size:9px;padding:1px 5px;border-radius:3px;font-weight:600;text-transform:uppercase;letter-spacing:.05em}
.kb-module{background:rgba(108,140,255,.2);color:var(--acc)}
.kb-class{background:rgba(255,216,102,.2);color:var(--yel)}
.kb-function,.kb-method{background:rgba(128,224,160,.2);color:var(--grn)}
.kb-group{background:rgba(108,140,255,.15);color:var(--acc)}
.kb-constant,.kb-variable{background:rgba(255,107,107,.2);color:var(--red)}
.kb-script{background:rgba(255,171,112,.2);color:var(--org)}
.kb-package{background:rgba(108,140,255,.2);color:var(--acc)}
.kb-external{background:rgba(139,143,167,.2);color:var(--dim)}
.kb-interface{background:rgba(128,224,160,.15);color:var(--grn)}
.nlbl{font-weight:600;color:var(--txt);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}

/* Node kinds */
.nk-group,.nk-module.is-overview{border-color:var(--acc);background:rgba(108,140,255,.06)}
.nk-group .node-hdr,.nk-module.is-overview .node-hdr{background:rgba(108,140,255,.1)}
.nk-external{background:rgba(139,143,167,.08);border-style:dashed;border-color:var(--dim)}
.nk-external:hover,.nk-external.selected{border-color:var(--dim);box-shadow:0 0 0 2px rgba(139,143,167,.2),0 4px 12px rgba(0,0,0,.3)}
...
.stereo{font-size:10px;color:var(--dim);font-style:italic}
.compart{padding:4px 12px;border-top:1px solid var(--bdr)}
.compart-item{font-size:12px;padding:2px 0;color:var(--txt)}
.compart-empty{font-size:11px;color:var(--dim);text-align:center;padding:2px 0}
.attr-i{color:var(--red);font-weight:700;margin-right:2px}
.meth-i{color:var(--grn);font-weight:700;margin-right:2px}
.type-h{color:var(--dim);font-size:11px}

/* Function signature */
.fn-sig{padding:2px 12px 4px;font-size:11px;color:var(--dim);font-family:"JetBrains Mono","Fira Code",monospace}
.fn-sig .pn{color:var(--txt)}
.fn-ret{padding:2px 12px 6px;font-size:11px;color:var(--grn);font-family:"JetBrains Mono","Fira Code",monospace}
...
.rb-reads{background:rgba(128,224,160,.18);color:var(--grn)}
.rb-writes{background:rgba(255,171,112,.18);color:var(--org)}
.rb-calls{background:rgba(108,140,255,.18);color:var(--acc)}
.rb-inherits,.rb-instantiates{background:rgba(255,216,102,.18);color:var(--yel)}
.rb-uses_config{background:rgba(139,143,167,.18);color:var(--dim)}
.rb-imports{background:rgba(128,224,160,.12);color:var(--grn)}

/* Drilldown */
.drilldown{padding:4px 12px 8px;font-size:11px;color:var(--acc);cursor:pointer}
.drilldown:hover{text-decoration:underline}
.node-count{padding:2px 12px 4px;font-size:10px;color:var(--dim)}
...
.et-calls{stroke:var(--acc);stroke-width:2}
.et-imports{stroke:var(--grn);stroke-dasharray:6 3}
.et-inherits{stroke:var(--yel);stroke-width:2}
.et-reads{stroke:var(--grn);stroke-dasharray:8 4;stroke-width:1.5}
.et-writes{stroke:var(--org);stroke-dasharray:8 4;stroke-width:1.5}
.et-instantiates{stroke:var(--yel);stroke-dasharray:4 2}
.et-uses_config,.et-uses-config{stroke:var(--dim);stroke-dasharray:3 6}
.et-multi{stroke-width:2.5;stroke-dasharray:10 4}
.edge-label{font-size:10px;fill:var(--dim);pointer-events:none}
.edge-arrow{fill:var(--dim)}
.et-calls+.edge-arrow,.et-calls~.edge-arrow{fill:var(--acc)}
marker.et-calls{fill:var(--acc)}

/* ── Inspector ── */
.insp{grid-area:insp;background:var(--bgp);border-left:1px solid var(--bdr);overflow-y:auto;padding:16px;font-size:12px}
.insp::-webkit-scrollbar{width:5px}
.insp::-webkit-scrollbar-thumb{background:var(--bdr);border-radius:4px}
.insp h2{font-size:15px;font-weight:600;margin-bottom:12px}
.insp-empty{color:var(--dim);font-size:13px;text-align:center;margin-top:40px}
.insp-card{background:var(--bgc);border-radius:8px;padding:12px;margin-bottom:12px}
.insp-card h3{font-size:13px;font-weight:600;margin-bottom:6px;color:var(--txt)}
.insp-row{display:flex;align-items:baseline;gap:6px;margin-bottom:4px;font-size:11px}
.insp-lbl{color:var(--dim);min-width:60px;flex-shrink:0}
.insp-val{color:var(--txt);word-break:break-word}
.insp-badge{font-size:9px;padding:2px 6px;border-radius:4px;font-weight:600;text-transform:uppercase;color:#0f1117}

/* Links */
.insp-link{color:var(--acc);cursor:pointer;font-size:11px}
.insp-link:hover{text-decoration:underline;color:#fff}
.insp-chips{display:flex;flex-wrap:wrap;gap:4px;margin-top:4px}
.insp-chip{display:inline-block;font-size:10px;padding:2px 8px;border-radius:4px;background:rgba(108,140,255,.12);color:var(--acc);cursor:pointer;transition:background .15s}
.insp-chip:hover{background:rgba(108,140,255,.3);color:#fff}
.insp-section{margin-top:8px;padding-top:8px;border-top:1px solid rgba(54,58,82,.5)}
.insp-section-lbl{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--acc);margin-bottom:4px}
.insp-summary{font-size:11px;line-height:1.5;color:var(--txt)}

/* Signature */
.insp-sig{background:rgba(0,0,0,.25);border:1px solid var(--bdr);border-radius:6px;padding:6px 10px;font-family:"JetBrains Mono","Fira Code",monospace;font-size:11px;overflow-x:auto;white-space:nowrap;margin-bottom:8px}
.sig-kw{color:#c9a0ff}
.sig-parens{color:var(--dim)}
.sig-params{color:#ff9070}
.sig-arrow{color:var(--dim)}
.sig-ret{color:var(--grn)}
...
.insp-tag{font-size:9px;padding:2px 6px;border-radius:3px;background:rgba(108,140,255,.15);color:var(--acc)}
.insp-tag-dead{background:rgba(255,80,80,.15);color:#ff5050}

/* ── Hover Card ── */
.hover-card{position:fixed;z-index:9999;width:380px;max-height:80vh;overflow-y:auto;background:#1e2030;border:1px solid #363a52;border-radius:10px;box-shadow:0 8px 32px rgba(0,0,0,.5),0 2px 8px rgba(0,0,0,.3);padding:12px 14px;font-size:12px;color:#c8d1e0;animation:hcIn .15s ease-out}
...
.hc-slbl{font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;color:var(--acc);margin-bottom:4px}
.hc-summary{font-size:11px;line-height:1.5}
.hc-chips{display:flex;flex-wrap:wrap;gap:4px}
.hc-chip{display:inline-block;font-size:10px;padding:2px 8px;border-radius:4px;background:rgba(108,140,255,.12);color:var(--acc);cursor:pointer;transition:background .15s;white-space:nowrap}
...
.hc-tag{font-size:9px;padding:2px 6px;border-radius:3px;background:rgba(108,140,255,.15);color:var(--acc)}
.hc-tag-dead{background:rgba(255,80,80,.15);color:#ff5050}
.hc-footer{margin-top:8px;padding-top:6px;border-top:1px solid rgba(54,58,82,.3);font-size:9px;color:var(--dim);text-align:center;opacity:.6}
...
vp.addEventListener("wheel", function(e) {
  ...
  zoom = Math.max(0.02, Math.min(5, zoom * delta));
...
function layoutView(view) {
  ...
  // Build adjacency from projected edges
  ...
  while (queue.length > 0) {
    ...
    queue.forEach(id => {
      (adj.get(id) || []).forEach(t => {
        ...
        if (inDeg.get(t) <= 0 && !visited.has(t)) {
          next.push(t);
          visited.add(t);
        }
      });
  ...
  layers.forEach(layer => {
    ...
    layer.forEach(id => {
      ...
      const w = estimateWidth(sym);
      const h = estimateHeight(sym);
      positions[id] = { x, y };
      x += w + GAP_X;
...
function estimateWidth(sym) {
  ...
  const base = Math.max(160, label.length * 8.5 + 48);
  if (sym.kind === "class") {
    const children = G.symbols.filter(s => s.parentId === sym.id);
    const maxChild = Math.max(0, ...children.map(c => (c.label || "").length * 8.5 + 48));
...
function estimateHeight(sym) {
  ...
  if (sym.kind === "function" || sym.kind === "method") {
    ...
    return 40 + (inputs.length > 0 ? 22 : 0) + 8;
  }
  return 66;
}

// ══════════════ Edge Projection (local copy) ══════════════
...
function projectEdgesForViewLocal(view) {
  ...
  edgeMap.forEach((agg, key) => {
    ...
    let label;
    if (agg.count === 1) {
      label = TYPE_VERBS[domType] || domType;
    } else if (entries.length === 1) {
      label = agg.count + "× " + (TYPE_VERBS[domType] || domType);
    } else {
      label = entries.sort((a, b) => b[1] - a[1]).map(([t, c]) => c + "× " + (TYPE_VERBS[t] || t)).join(", ");
    }
    const animated = ["calls", "instantiates"].includes(domType);
    result.push({ key, source: agg.source, target: agg.target, type: domType, count: agg.count, label, animated, relationIds: agg.relationIds, typeCounts: agg.typeCounts, className: "et-" + domType + (entries.length > 1 ? " et-multi" : "") });
  });
  return result;
}
...
function renderView(viewId) {
  ...
  ["dim","acc","grn","org","yel"].forEach(c => {
    const colors = { dim:"#8b8fa7", acc:"#6c8cff", grn:"#80e0a0", org:"#ffab70", yel:"#ffd866" };
  ...
  edges.forEach(e => {
    ...
    if (!sp || !tp) return;
    const srcSym = symMap.get(e.source);
    const tgtSym = symMap.get(e.target);
    const sw = estimateWidth(srcSym);
    const sh = estimateHeight(srcSym);
    const tw = estimateWidth(tgtSym);
    const th = estimateHeight(tgtSym);
    ...
    const arrowColor = typeToArrow[e.type] || "dim";
    path.setAttribute("marker-end", "url(#arrow-" + arrowColor + ")");
  ...
  view.nodeRefs.forEach(symId => {
    ...
    if (sym.kind === "class") {
      el.innerHTML = renderClassNode(sym, scope);
    } else if (sym.kind === "function" || sym.kind === "method") {
      el.innerHTML = renderFunctionNode(sym);
    } else if (sym.kind === "external") {
      el.innerHTML = renderArtifactNode(sym);
    } else {
      el.innerHTML = renderDefaultNode(sym, scope);
    }
...
function renderDefaultNode(sym, scope) {
  ...
  // Children count for groups/modules
...
function renderClassNode(sym, scope) {
  const children = G.symbols.filter(s => s.parentId === sym.id);
  const attrs = children.filter(c => c.kind === "constant" || c.kind === "variable");
  const meths = children.filter(c => c.kind === "method" || c.kind === "function");
  ...
  if (attrs.length > 0) {
    ...
    html += '<div class="compart-empty">—</div>';
  }
  html += '</div>';
...
function renderFunctionNode(sym) {
  ...
  if (outputs.length > 0) {
    html += '<div class="fn-ret">→ ' + outputs.map(o => o.type || o.name).join(", ") + '</div>';
  }
...
function renderDrilldown(sym) {
  return '<div class="drilldown" onclick="event.stopPropagation(); navigateToView(\\'' + sym.childViewId + '\\')">' + (sym.kind === "class" ? "▶ Methods detail" : "▶ Drill down") + '</div>';
}
...
function renderSidebar() {
  ...
  sb.innerHTML = '<h2>Views</h2><div id="viewTree"></div>';
  ...
  function renderTreeItem(v, level, container) {
    ...
    item.className = "vt-item" + (isActive ? " active" : "");
    ...
    if (hasKids) {
      ...
      children.forEach(c => renderTreeItem(c, level + 1, childContainer));
...
function renderBreadcrumb() {
  ...
  // Build path from root to current view
  const path = [];
...
function renderInspector() {
  ...
  // Location
  if (loc) {
    const lineCount = (loc.startLine != null && loc.endLine != null) ? (loc.endLine - loc.startLine + 1) : null;
    html += '<div class="insp-row"><span class="insp-lbl">📄 Datei</span><span class="insp-val" style="font-family:monospace;font-size:10px">' + esc(loc.file) + (loc.startLine != null ? ":" + loc.startLine : "") + (loc.endLine != null ? "-" + loc.endLine : "") + (lineCount ? " (" + lineCount + " Zeilen)" : "") + '</span></div>';
  }
  ...
  // Relations cards
  function relCard(title, icon, items, useTarget) {
  ...
  html += relCard("Ruft auf", "→", outCalls, true);
  html += relCard("Aufgerufen von", "←", inCalls, false);
  html += relCard("Liest", "📖", reads, true);
  html += relCard("Schreibt", "📝", writes, true);
  html += relCard("Importiert", "📥", imports, true);
  html += relCard("Importiert von", "📤", importedBy, false);
  html += relCard("Erbt von", "🧬", inherits, true);
  html += relCard("Instanziiert", "🏗", instantiates, true);
  ...
  if (children.length > 0) {
    ...
    if (children.length > 20) html += '<span style="color:var(--dim);font-size:10px">+' + (children.length - 20) + ' weitere</span>';
    html += '</div></div>';
  }
...
function navigateToSymbol(symId) {
  // Find deepest view containing this symbol
  let bestView = null;
  G.views.forEach(v => {
  ...
  if (bestView && bestView !== currentViewId) {
    renderView(bestView);
  }
...
function showHoverCard(symId, rect) {
  ...
  rels.forEach(r => {
    if (r.source === sym.id) {
      if (!outgoing[r.type]) outgoing[r.type] = [];
      outgoing[r.type].push(symMap.get(r.target));
    } else {
      if (!incoming[r.type]) incoming[r.type] = [];
      incoming[r.type].push(symMap.get(r.source));
    }
  });
  ...
  if (children.length > 0) {
    ...
    if (children.length > 12) html += '<span style="color:var(--dim);font-size:10px">+' + (children.length - 12) + ' weitere</span>';
    html += '</div></div>';
  }
